    ...
    <div className="gig flex justify-center lg:mt-32">
      <div className="container lg:w-[90%] py-7 px-0 flex gap-12">
        <div className="left flex-grow-[1] flex-shrink flex-basis-0 flex flex-col gap-5">
          <span className="breadcrumbs font-light uppercase text-xs text-gray-500">
          ...
          <p className="font-light text-gray-500">{data?.desc}</p>
          <Reviews gigId={id!} />
        </div>
        <div className="right border border-gray-300 p-5 rounded-sm flex flex-col  gap-5 h-max   absolute top-[150px] right-5 w-[400px]">
          ...
          <div className="features">
            {data?.features?.map((feature) => (
              <div className="item flex items-center gap-2 font-light text-gray-500 mb-1" key={feature}>
          ...
          <Link to={`/pay/${id}`}>
            <button className="bg-green-500 p-2 text-white font-medium border-none text-xl cursor-pointer">Continue</button>
          </Link>
        </div>
      </div>
    </div>
